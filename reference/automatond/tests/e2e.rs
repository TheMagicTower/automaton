...
struct Scripted {
    turns: Mutex<Vec<Vec<StreamItem>>>,
    call: Mutex<usize>,
}
    ...
    let approval = evs
        ...
        .find_map(|e| match e {
            Event::ApprovalRequested {
                approval, action, ..
            } if action.tool == "mode.switch" => Some(approval.clone()),
            _ => None,
        })
        .expect("모드 전환 승인 요청(mode.switch) 미수신");
    let resp = format!(
        "{{\"method\":\"approval_respond\",\"params\":{{\"session\":\"{session}\",\"approval\":\"{approval}\",\"decision\":\"approve\",\"always\":false}}}}\n"
    );
    wr.write_all(resp.as_bytes()).await.unwrap();
    let evs = read_events(reader, "ModeChanged").await;
    assert!(
        evs.iter()
            .any(|e| matches!(e, Event::ModeChanged { mode, .. } if *mode == expect)),
        "승인 후 모드 적용 실패"
    );
}
...
async fn allow_path_runs_tool_streams_and_audits() {
    ...
    let provider = Scripted {
        turns: Mutex::new(vec![
            vec![StreamItem::ToolCall(automaton_core::ToolCall {
                name: "fs.write".into(),
                args: json!({"path": &target, "content": "brass"}),
            })],
    ...
    wr.write_all(
        "{\"method\":\"message_send\",\"params\":{\"session\":\"s1\",\"text\":\"기록해\"}}\n"
            .as_bytes(),
    )
    .await
    .unwrap();
    ...
    assert!(
        evs.iter()
            .any(|e| matches!(e, Event::ToolStarted { tool, .. } if tool == "fs.write"))
    );
    assert!(
        evs.iter()
    ...
    assert!(
        rows >= 2,
        "세션 메시지 미저장 또는 재기록 결함 (rows={rows})"
    );
}
...
async fn usage_reported_and_written_to_audit_log() {
    ...
    let provider = Scripted {
        turns: Mutex::new(vec![vec![
            ...
            StreamItem::Usage(automaton_proto::TokenUsage {
                prompt_tokens: 11,
                completion_tokens: 4,
                total_tokens: 15,
            }),
    ...
    wr.write_all(b"{\"method\":\"session_create\",\"params\":{\"id\":\"s1\"}}\n")
        ...
        .unwrap();
    wr.write_all(
        "{\"method\":\"message_send\",\"params\":{\"session\":\"s1\",\"text\":\"안녕\"}}\n"
            .as_bytes(),
    )
    .await
    .unwrap();
    ...
    assert!(
        evs.iter()
            .any(|e| matches!(e, Event::Usage { usage, .. } if usage.total_tokens == 15))
    );
    ...
    for _ in 0..20 {
        if let Ok(a) = std::fs::read_to_string(&audit_path)
            && a.contains("\"type\":\"usage\"")
            && a.contains("\"total_tokens\":15")
        {
            audited = true;
            break;
        }
...
async fn ask_path_requires_approval_roundtrip() {
    ...
    approve_mode_switch(&mut reader, &mut wr, "s1", Mode::Code).await; // F-03: 데몬 측 승인 게이트
    wr.write_all(
        "{\"method\":\"message_send\",\"params\":{\"session\":\"s1\",\"text\":\"삭제해\"}}\n"
            .as_bytes(),
    )
    .await
    .unwrap();
    ...
    let resp = format!(
        "{{\"method\":\"approval_respond\",\"params\":{{\"session\":\"s1\",\"approval\":\"{approval}\",\"decision\":\"approve\",\"always\":false}}}}\n"
    );
    wr.write_all(resp.as_bytes()).await.unwrap();
    ...
    assert!(
        evs.iter()
            .any(|e| matches!(e, Event::ToolStarted { tool, .. } if tool == "fs.delete"))
    );
    assert!(
        evs.iter()
            .any(|e| matches!(e, Event::ToolResult { ok: true, .. }))
...
async fn approval_after_three_approvals_emits_draft_suggestions() {
    ...
    let targets: Vec<_> = (0..4)
        ...
        .collect();
    let tool = |i: usize| {
        StreamItem::ToolCall(automaton_core::ToolCall {
            name: "fs.delete".into(),
            args: json!({"path": &targets[i]}),
        })
    };
    let provider = Scripted {
        turns: Mutex::new(vec![
    ...
    let idx_approval = evs
        .iter()
        .position(|e| matches!(e, Event::ApprovalRequested { .. }))
        .expect("배너가 초안보다 먼저");
    let idx_drafts = evs
        .iter()
        .position(|e| matches!(e, Event::DraftSuggestions { .. }))
        .expect("DraftSuggestions 미수신");
    assert!(
        idx_approval < idx_drafts,
        "DraftSuggestions는 ApprovalRequested 직후에 발행되어야 함"
    );
    match &evs[idx_drafts] {
        Event::DraftSuggestions {
            session,
            suggestions,
        } => {
            assert_eq!(session, "s1");
            assert!(
                suggestions.iter().any(|s| s == "진행해"),
                "초안에 '진행해' 필요: {suggestions:?}"
            );
        }
        _ => unreachable!(),
    }
...
async fn mode_switch_denial_keeps_session_mode_f03() {
    ...
    let provider = Scripted {
        turns: Mutex::new(vec![
            vec![StreamItem::ToolCall(automaton_core::ToolCall {
                name: "fs.write".into(),
                args: json!({"path": &target, "content": "x"}),
            })],
            vec![StreamItem::Delta("완료".into())],
        ]),
        call: Mutex::new(0),
    ...
    assert!(
        evs.iter().any(
            |e| matches!(e, Event::Error { message, .. } if message.contains("모드 전환 거절"))
        ),
        "거절 응답 이벤트 미수신: {evs:?}"
    );
    ...
    assert!(
        evs.iter().any(
            |e| matches!(e, Event::ApprovalRequested { action, .. } if action.tool == "fs.write")
        ),
        "chat 유지 시 fs.write는 ASK여야 함: {evs:?}"
    );
    assert!(
        !target.exists(),
        "거절된 모드 전환 후 파일이 자동 기록되면 결함"
    );
}
...
async fn approval_response_from_other_session_rejected_f03() {
    ...
    let approval = evs
        ...
        .find_map(|e| match e {
            Event::ApprovalRequested {
                approval, action, ..
            } if action.tool == "fs.delete" => Some(approval.clone()),
            _ => None,
        })
    ...
    assert!(
        evs.iter()
            .any(|e| matches!(e, Event::Error { message, .. } if message.contains("세션"))),
        "세션 바인딩 위반 오류 미수신: {evs:?}"
    );
...
async fn session_gate_keys_waiters_by_session_and_tool_f05() {
    ...
    assert!(
        matches!(dup.await.unwrap(), ApprovalOutcome::Deny),
        "중복 ASK는 덮어쓰지 않고 즉시 거부"
    );

    // s1 승인이 s2 waiter를 건드리지 않는다
    gate.pending
        .lock()
        .remove("s1:shell.exec")
        .unwrap()
        .send(Decision::Approve)
        .unwrap();
    assert!(matches!(a.await.unwrap(), ApprovalOutcome::Approve));
    assert!(
        gate.pending.lock().contains_key("s2:shell.exec"),
        "타 세션 waiter 잔존"
    );
