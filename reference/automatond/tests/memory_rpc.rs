...
impl Provider for Capturing {
    async fn complete(&self, req: CompletionRequest) -> Result<Vec<StreamItem>, CoreError> {
        ...
        if req.system.contains("요약 도우미") {
            Ok(vec![StreamItem::Delta(
                "다운로드 폴더 정리를 마쳤고 vim 설정을 바꿨다".into(),
            )])
        } else {
...
async fn memory_browse_stats_delete_roundtrip() {
    ...
    // 데몬 기동 전 시드 — 데몬은 같은 DB 파일을 다시 연다 (WAL + busy_timeout으로 다중 연결 안전)
    {
        ...
        seed.record_decision("s0", "fs.delete", "x.zip", "ask", "approve")
            .unwrap();
        seed.save_summary("s0", "다운로드 폴더 정리를 마쳤고 vim 설정을 바꿨다")
            .unwrap();
    }
    ...
    let mut stream = None;
    for _ in 0..20 {
        if let Ok(s) = tokio::net::UnixStream::connect(root.join("d.sock")).await {
            stream = Some(s);
            break;
        }
    ...
    match evs.last() {
        Some(Event::MemoryData { facts, total }) => {
            assert_eq!(
                facts.as_slice(),
                ["caspar는 한국어를 쓴다".to_string()].as_slice(),
                "첫 페이지: {facts:?}"
            );
    ...
    match evs.last() {
        Some(Event::MemoryData { facts, total }) => {
            assert_eq!(
                facts.as_slice(),
                ["선호 에디터는 vim".to_string()].as_slice(),
                "둘째 페이지: {facts:?}"
            );
            assert_eq!(*total, 2);
        }
    ...
    wr.write_all(
        "{\"method\":\"memory_delete\",\"params\":{\"content\":\"caspar는 한국어를 쓴다\"}}\n"
            .as_bytes(),
    )
    .await
    .unwrap();
    let evs = read_events(&mut reader, "MemoryData").await;
    assert!(
        matches!(evs.last(), Some(Event::MemoryData { facts, total }) if facts.is_empty() && *total == 1),
        "삭제 확인: {evs:?}"
    );
    ...
    let evs = read_events(&mut reader, "MemoryData").await;
    assert!(
        matches!(evs.last(), Some(Event::MemoryData { facts, total }) if facts.len() == 1 && facts[0] == "선호 에디터는 vim" && *total == 1),
        "삭제 후 재검색: {evs:?}"
    );

    // 4) 없는 fact 삭제 — 무음 드롭 금지, 명시적 오류
    wr.write_all(
        "{\"method\":\"memory_delete\",\"params\":{\"content\":\"없는 fact\"}}\n".as_bytes(),
    )
    .await
    .unwrap();
    let evs = read_events(&mut reader, "Error").await;
    assert!(
        matches!(evs.last(), Some(Event::Error { message, .. }) if message.contains("삭제할 fact가 없음")),
        "명시적 오류: {evs:?}"
    );
    ...
    assert!(
        matches!(evs.last(), Some(Event::SummaryData { session, summary }) if session == "s0" && summary == "다운로드 폴더 정리를 마쳤고 vim 설정을 바꿨다"),
        "요약 왕복: {evs:?}"
    );
    ...
    let evs = read_events(&mut reader, "SummaryData").await;
    assert!(
        matches!(evs.last(), Some(Event::SummaryData { summary, .. }) if summary.is_empty()),
        "요약 없음 = 빈 문자열: {evs:?}"
    );
}
...
async fn session_summarized_after_idle_and_injected_into_next_session_prompt() {
    ...
    for _ in 0..60 {
        ...
        if let Ok(s) = MemoryStore::open(&db)
            && let Some(first) = s.search_summaries("정리").unwrap().first()
        {
    ...
    wr.write_all(b"{\"method\":\"session_create\",\"params\":{\"id\":\"s2\"}}\n")
        ...
        .unwrap();
    wr.write_all(
        "{\"method\":\"message_send\",\"params\":{\"session\":\"s2\",\"text\":\"새 작업 시작\"}}\n"
            .as_bytes(),
    )
    .await
    .unwrap();
    ...
    let chat_prompts: Vec<String> = systems
        .lock()
        .iter()
        .filter(|s| !s.contains("요약 도우미"))
        .cloned()
        .collect();
    assert!(
        chat_prompts.len() >= 2,
        "채팅 호출 2회 이상이어야 함: {chat_prompts:?}"
    );
    assert!(
        !chat_prompts[0].contains("이전 세션 요약"),
        "s1 프롬프트에는 요약이 없어야 함"
    );
    assert!(
        chat_prompts[1].contains("## 이전 세션 요약"),
        "s2 프롬프트에 요약 섹션 필요: {}",
        chat_prompts[1]
    );
    assert!(
        chat_prompts[1].contains(&saved),
        "s2 프롬프트가 저장된 요약을 포함해야 함: {saved}"
    );
}
...
async fn new_session_start_forces_pending_summary_without_idle_wait() {
    ...
    for _ in 0..40 {
        tokio::time::sleep(Duration::from_millis(50)).await;
        if let Ok(s) = MemoryStore::open(&db)
            && !s.search_summaries("vim").unwrap().is_empty()
        {
            found = true;
            break;
        }
    ...
    assert!(
        found,
        "새 세션 시작이 유휴 대기 없이 s1 요약을 강제 트리거해야 함"
    );
}
