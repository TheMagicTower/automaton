...
async fn main() {
    ...
    match cmd.as_str() {
        "doctor" => doctor(&paths),
        "serve" => {
            let socket = PathBuf::from(args.next().unwrap_or_else(|| {
                paths
                    .data_dir
                    .join("automatond.sock")
                    .to_string_lossy()
                    .into()
            }));
            ...
            let provider: Box<dyn Provider> = match automaton_core::OpenAiCompat::from_env() {
                Some(p) => Box::new(p),
                None => {
                    eprintln!("AUTOMATON_API_KEY 미설정 — .env 또는 키체인 설정 후 재시도");
                    std::process::exit(2);
                }
            };
        ...
        other => {
            eprintln!("모름: {other} · 사용법: automatond [serve [소켓경로]|doctor]");
            std::process::exit(2);
        }
    }
}
...
fn doctor(paths: &Paths) {
    ...
    let ax = std::process::Command::new("osascript")
        .arg("-e")
        .arg("tell application \"System Events\" to name of first process")
        .output();
    println!(
        "접근성 권한: {}",
        match ax {
            Ok(o) if o.status.success() => "정상",
            Ok(_) => "거부됨 — 시스템 설정>개인정보>접근성에서 automatond 허용",
            Err(_) => "osascript 없음",
        }
    );
    ...
    let cap = std::process::Command::new("screencapture")
        .arg("-x")
        .arg("/tmp/automaton-doctor.png")
        .output();
    println!(
        "스크린 레코딩: {}",
        match cap {
            Ok(o) if o.status.success() =>
                "정상(권한 거부 시에도 exit 0일 수 있음 — /tmp/automaton-doctor.png 내용으로 확인)",
            _ => "거부됨 — 시스템 설정>개인정보>화면 기록에서 허용",
        }
    );
    println!(
        "AUTOMATON_API_KEY: {}",
        if std::env::var("AUTOMATON_API_KEY").is_ok() {
            "설정됨"
        } else {
            "미설정"
        }
