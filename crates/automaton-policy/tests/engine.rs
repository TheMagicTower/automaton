...
fn builtin_allows_reads_in_every_mode() {
    for mode in [Mode::Code, Mode::Mac, Mode::Chat] {
        ...
        assert!(matches!(
            e.evaluate(&action("fs.read", Category::Read), mode),
            Verdict::Allow
        ));
        assert!(matches!(
            e.evaluate(&action("capture.screen", Category::Read), mode),
            Verdict::Allow
        ));
    }
}
...
fn builtin_allows_code_mode_edits_but_asks_in_other_modes() {
    let e = Engine::builtin();
    assert!(matches!(
        e.evaluate(&action("fs.write", Category::Write), Mode::Code),
        Verdict::Allow
    ));
    assert!(matches!(
        e.evaluate(&action("fs.write", Category::Write), Mode::Mac),
        Verdict::Ask { .. }
    ));
}
...
fn destructive_and_external_actions_always_ask() {
    for mode in [Mode::Code, Mode::Mac, Mode::Chat] {
        let e = Engine::builtin();
        assert!(matches!(
            e.evaluate(&action("fs.delete", Category::Destructive), mode),
            Verdict::Ask { .. }
        ));
        assert!(matches!(
            e.evaluate(&action("shell.exec", Category::External), mode),
            Verdict::Ask { .. }
        ));
    }
}
...
fn input_defaults_to_ask_even_in_mac_mode_until_granted() {
    ...
    let a = Action {
        tool: "input.type".into(),
        category: Category::Input,
        app: Some("com.apple.finder".into()),
        target: Some("search field".into()),
    };
    assert!(matches!(e.evaluate(&a, Mode::Mac), Verdict::Ask { .. }));
    e.grant_always(Rule {
        name: "finder-input".into(),
        tool: Some("input.type".into()),
        app: Some("com.apple.finder".into()),
        category: None,
...
fn input_without_target_never_allows_even_when_granted_f04() {
    ...
    for category in [Category::Input, Category::External] {
        // 툴 분류와 무관하게 정책이 최종 방어
        let a = Action {
            tool: "input.type".into(),
            category,
            app: Some("com.apple.finder".into()),
        ...
        for mode in [Mode::Code, Mode::Mac, Mode::Chat] {
            assert!(
                matches!(e.evaluate(&a, mode), Verdict::Ask { .. }),
                "target 미포함 input은 granted로도 Allow 불가 ({category:?}/{mode:?})"
            );
        }
    }
}
...
fn sensitive_targets_are_denied_regardless_of_rules() {
    ...
    let a = Action {
        tool: "input.type".into(),
        category: Category::Input,
        app: Some("com.some.bank".into()),
        target: Some("SecureTextField".into()),
    };
...
fn sensitive_targets_denied_case_insensitive() {
    ...
    for target in [
        ...
        assert!(
            matches!(e.evaluate(&a, Mode::Mac), Verdict::Deny { .. }),
            "should deny target: {target}"
        );
    }
    ...
    for benign in ["typing_area", "shipping_address", "spinner_control"] {
        let a = Action {
            ...
            category: Category::Input,
            app: Some("com.apple.finder".into()),
            target: Some(benign.into()),
        };
        assert!(
            matches!(e.evaluate(&a, Mode::Mac), Verdict::Ask { .. }),
            "should not deny benign: {benign}"
        );
    }
}
...
fn mode_switch_always_asks_even_when_granted() {
    let e = Engine::builtin();
    assert!(matches!(
        e.evaluate(&action("mode.switch", Category::ModeSwitch), Mode::Chat),
        Verdict::Ask { .. }
    ));
    // §5 원천 차단 불변식: 소유자 granted Allow 규칙으로도 모드 전환은 우회 불가
    let mut g = Engine::builtin();
    g.grant_always(Rule {
        name: "always-switch".into(),
        tool: Some("mode.switch".into()),
        app: None,
        category: None,
        verdict: VerdictTemplate::Allow,
    });
    assert!(matches!(
        g.evaluate(&action("mode.switch", Category::ModeSwitch), Mode::Chat),
        Verdict::Ask { .. }
    ));
}
...
fn grant_always_persists_and_allows_future_matching() {
    ...
    assert!(matches!(e.evaluate(&a, Mode::Mac), Verdict::Ask { .. }));
    e.grant_always(Rule {
        name: "trash-downloads".into(),
        tool: Some("fs.delete".into()),
        app: None,
        category: Some(Category::Destructive),
        verdict: VerdictTemplate::Allow,
    });
    assert!(matches!(e.evaluate(&a, Mode::Mac), Verdict::Allow));
}
...
fn policy_file_roundtrip_persists_granted_rules() {
    ...
    assert!(matches!(
        loaded.evaluate(&action("fs.delete", Category::Destructive), Mode::Mac),
        Verdict::Allow
    ));
}
