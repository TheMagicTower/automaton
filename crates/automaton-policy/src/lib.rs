...
impl Engine {
    ...
    pub fn builtin() -> Self {
        let ask = |name: &str, category: Category| Rule {
            ...
            verdict: VerdictTemplate::Ask,
        };
        let deny_app = |app: &str| Rule {
            name: format!("deny-app-{app}"),
            tool: None,
            app: Some(app.into()),
            category: None,
            verdict: VerdictTemplate::Deny,
        };
        Engine {
            granted: vec![],
            rules: vec![
    ...
    /// 결정론적 평가 순서:
    /// - 비밀번호 필드 → 정적 DENY (그 무엇도 우회 불가)
    /// - 입력 툴 target 미포함 → 항상 ASK (granted로도 우회 불가, F-04)
    /// - 모드 전환 → 항상 ASK (granted로도 우회 불가 — §5 원천 차단)
    /// - 소유자 명시 granted → Allow (§5 민감 영역 명시 등록 = 화이트리스트)
    /// - 명시 DENY 규칙 (규칙 목록 내 Allow보다 항상 우선)
    /// - 나머지 규칙 첫 매치
    /// - 카테고리×모드 기본값
    pub fn evaluate(&self, a: &Action, mode: Mode) -> Verdict {
        ...
        if let Some(t) = &a.target {
            ...
            let is_sensitive = lower.contains("securetextfield")
                ...
                || lower
                    ...
                    .any(|w| matches!(w, "pin" | "otp" | "cvv"));
            if is_sensitive {
                return Verdict::Deny {
                    reason: format!("민감 입력 필드: {t}"),
                };
            }
        }
        // 1.5 입력 툴 target 미포함 (F-04) — target은 모델 자기선언이라 누락 시 민감 필드 검사가
        // 우회된다. 어떤 granted·규칙으로도 Allow 불가, 항상 ASK.
        if a.tool.starts_with("input.") && a.target.as_deref().is_none_or(|t| t.trim().is_empty()) {
            return Verdict::Ask {
                reason: format!("{} target 미포함 — 민감 필드 검사 불가", short(a)),
            };
        }
        ...
        if self
            .granted
            .iter()
            .any(|r| r.verdict == VerdictTemplate::Allow && r.app.is_some() && matches(r, a))
        {
        ...
        if let Some(r) = self
            .rules
            .iter()
            .find(|r| r.verdict == VerdictTemplate::Deny && matches(r, a))
        {
            return Verdict::Deny {
                reason: format!("규칙 {}: 거부", r.name),
            };
        }
        ...
        if self
            .granted
            .iter()
            .any(|r| r.verdict == VerdictTemplate::Allow && matches(r, a))
        {
            return Verdict::Allow;
        }
        // 5. 나머지 규칙 — 첫 매치
        ...
        match (a.category, mode) {
            ...
            (Category::Write, _) => Verdict::Ask {
                reason: format!("{} 쓰기", short(a)),
            },
            (Category::Destructive, _) => Verdict::Ask {
                reason: format!("{} 파괴 동작", short(a)),
            },
            (Category::External, _) => Verdict::Ask {
                reason: format!("{} 외부 실행", short(a)),
            },
            (Category::System, _) => Verdict::Ask {
                reason: format!("{} 시스템 변경", short(a)),
            },
            (Category::Input, _) => Verdict::Ask {
                reason: format!("{} 미등록 앱 입력", short(a)),
            },
    ...
    pub fn grant_always(&mut self, rule: Rule) {
...
fn matches(r: &Rule, a: &Action) -> bool {
    let tool_ok = r.tool.as_ref().is_none_or(|t| t == &a.tool);
    let app_ok = r.app.as_ref().is_none_or(|g| a.app.as_ref() == Some(g));
    let cat_ok = r.category.is_none_or(|c| c == a.category);
...
fn from_template(t: &VerdictTemplate, a: &Action) -> Verdict {
    match t {
        ...
        VerdictTemplate::Ask => Verdict::Ask {
            reason: format!("{} 규칙 승인 필요", short(a)),
        },
        VerdictTemplate::Deny => Verdict::Deny {
            reason: format!("{} 규칙 거부", short(a)),
        },
    }
}

fn short(a: &Action) -> &str {
    &a.tool
}
