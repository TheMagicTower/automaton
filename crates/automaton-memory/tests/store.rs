...
fn decisions_record_and_fts_search() {
    ...
    for i in 0..3 {
        s.record_decision(
            "s1",
            "fs.delete",
            &format!("~/Downloads/old-{i}.zip"),
            "ask",
            "approve",
        )
        .unwrap();
    }
    s.record_decision("s2", "fs.delete", "~/Documents/plan.md", "ask", "deny")
        .unwrap();
...
fn extract_keywords_strips_korean_particles() {
    ...
    assert!(
        kw.contains(&"caspar".to_string()),
        "조사 '의' 제거 실패: {kw:?}"
    );
    assert!(
        kw.contains(&"이름".to_string()),
        "조사 '이' 제거 실패: {kw:?}"
    );
    assert!(
        !kw.contains(&"집에서는".to_string()),
        "복합 조사 '에서는'이 통째로 남음: {kw:?}"
    );
    assert!(
        !kw.iter().any(|k| k.chars().count() < 2),
        "1글자 토큰이 남음: {kw:?}"
    );
}
...
fn related_search_matches_despite_particles() {
    ...
    assert!(
        hits.iter().any(|f| f.contains("카스파르")),
        "조사 붙은 질의가 이름 fact에 걸려야 함: {hits:?}"
    );
    assert!(
        !hits.iter().any(|f| f.contains("vim")),
        "무관 fact는 뒤로/제외: {hits:?}"
    );
}
...
fn related_search_expands_synonyms() {
    ...
    assert!(
        hits.iter().any(|f| f.contains("caspar")),
        "동의어 확장으로 성함 fact 검색되어야 함: {hits:?}"
    );
}
...
fn facts_page_delete_stats_roundtrip() {
    ...
    s.append_message("s1", "user", "안녕").unwrap();
    s.record_decision("s1", "fs.delete", "x", "ask", "approve")
        .unwrap();
