...
} // parking_lot — 즉시 unwrap 락은 rs-parking-lot 룰 적용 + rusqlite Connection !Sync의 Sync화 (계획 수정 8d94474)
...
impl MemoryStore {
    ...
    pub fn messages(&self, session: &str) -> Result<Vec<Message>> {
        ...
        let mut stmt =
            conn.prepare("SELECT role, content FROM messages WHERE session = ?1 ORDER BY id")?;
        let rows = stmt.query_map([session], |r: &Row| {
            Ok(Message {
                role: r.get(0)?,
                content: r.get(1)?,
            })
    ...
    pub fn recent_summaries(&self, limit: usize, exclude_session: &str) -> Result<Vec<String>> {
        ...
        let rows = stmt.query_map(params![exclude_session, limit as i64], |r| {
            r.get::<_, String>(0)
        })?;
        Ok(rows.collect::<std::result::Result<Vec<_>, _>>()?)
    }
    ...
    pub fn add_fact(&self, content: &str) -> Result<()> {
        self.conn
            .lock()
            .execute("INSERT INTO facts_fts(content) VALUES (?1)", (content,))?;
        Ok(())
    }
    ...
    pub fn search_facts_related(&self, query: &str, limit: usize) -> Result<Vec<String>> {
        ...
        if keywords.is_empty() {
            return self.recent_facts(limit);
        }
        ...
        let mut scored = {
            let conn = self.conn.lock();
            let mut stmt =
                conn.prepare("SELECT rowid, content FROM facts_fts WHERE facts_fts MATCH ?1")?;
            ...
            for row in rows {
                ...
                let score: u32 = terms
                    .iter()
                    .map(|(t, primary)| {
                        lower.contains(t.as_str()) as u32 * if *primary { 2 } else { 1 }
                    })
                    .sum();
    ...
    pub fn recent_facts(&self, limit: usize) -> Result<Vec<String>> {
        let conn = self.conn.lock();
        let mut stmt =
            conn.prepare("SELECT content FROM facts_fts ORDER BY rowid DESC LIMIT ?1")?;
    ...
    pub fn facts_page(&self, offset: usize, limit: usize) -> Result<(Vec<String>, usize)> {
        ...
        let mut stmt =
            conn.prepare("SELECT content FROM facts_fts ORDER BY rowid LIMIT ?1 OFFSET ?2")?;
    ...
    pub fn delete_fact(&self, content: &str) -> Result<usize> {
        Ok(self
            .conn
            .lock()
            .execute("DELETE FROM facts_fts WHERE content = ?1", (content,))?)
    }
    ...
    pub fn search_decisions(&self, query: &str) -> Result<Vec<Decision>> {
        ...
        let rows = stmt.query_map([fts_escape(query)], |r: &Row| {
            Ok(Decision {
                session: r.get(0)?,
                tool: r.get(1)?,
                target: r.get(2)?,
    ...
    pub fn search_decisions_fts(&self, raw_fts_query: &str) -> Result<Vec<Decision>> {
        let conn = self.conn.lock();
        let mut stmt = conn.prepare("SELECT session, tool, target, verdict, decision FROM decisions_fts WHERE decisions_fts MATCH ?1")?;
        let rows = stmt.query_map([raw_fts_query], |r: &Row| {
            Ok(Decision {
                session: r.get(0)?,
                tool: r.get(1)?,
                target: r.get(2)?,
    ...
    pub fn decisions_by_tool(&self, tool: &str) -> Result<Vec<Decision>> {
        ...
        let mut stmt = conn.prepare(
            "SELECT session, tool, target, verdict, decision FROM decisions_fts WHERE tool = ?1",
        )?;
        let rows = stmt.query_map([tool], |r: &Row| {
            Ok(Decision {
                session: r.get(0)?,
                tool: r.get(1)?,
                target: r.get(2)?,
...
pub fn extract_keywords(query: &str) -> Vec<String> {
    ...
    for tok in query.split(|c: char| !c.is_alphanumeric()) {
        ...
        if k.chars().count() >= 2 && !out.contains(&k) {
            out.push(k);
        }
    }
    out
