...
fn draft_suggestions_event_roundtrip() {
    let ev = Event::DraftSuggestions {
        session: "s1".into(),
        suggestions: vec!["진행해".into(), "허용".into()],
    };
    let json = serde_json::to_string(&ev).unwrap();
...
fn memory_rpc_roundtrip() {
    ...
    let req = Request::MemoryDelete {
        content: "caspar는 한국어를 쓴다".into(),
    };
    let json = serde_json::to_string(&req).unwrap();
    ...
    let ev = Event::MemoryData {
        facts: vec!["caspar는 한국어를 쓴다".into()],
        total: 1,
    };
    let json = serde_json::to_string(&ev).unwrap();
...
fn summary_rpc_roundtrip() {
    ...
    let ev = Event::SummaryData {
        session: "shell-1".into(),
        summary: "다운로드 폴더 정리를 마쳤다".into(),
    };
    let json = serde_json::to_string(&ev).unwrap();
    ...
    let ev = Event::SummaryData {
        session: "shell-2".into(),
        summary: String::new(),
    };
    assert_eq!(
        serde_json::from_str::<Event>(&serde_json::to_string(&ev).unwrap()).unwrap(),
        ev
    );
}
