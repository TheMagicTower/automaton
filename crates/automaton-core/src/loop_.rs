...
use crate::provider::{
    CompletionRequest, CoreError, Message, Provider, ProviderStream, StreamItem, ToolCall,
};
use automaton_policy::{Action, Engine, Verdict};
use automaton_proto::{ActionInfo, Event, Mode};
use automaton_tools::{Registry, Tool, action_context};
...
impl<G: ApprovalGate + ?Sized> ApprovalGate for Box<G> {
    async fn decide(&self, session: &str, action: ActionInfo) -> ApprovalOutcome {
        (**self).decide(session, action).await
    }
}
...
impl<P: Provider, G: ApprovalGate> AgentLoop<P, G> {
        ...
        for _ in 0..MAX_TURNS {
            ...
            while let Some(item) = stream.next().await {
                match item? {
                    ...
                    StreamItem::Usage(u) => emit(Event::Usage {
                        session: session.clone(),
                        usage: u,
                    }),
                }
            ...
            for call in calls {
                ...
                if consecutive_failures >= 3 {
                    return Err(CoreError::Message(
                        "동일 툴 연속 3회 실패 — 중단 (§9)".into(),
                    ));
                }
            }
        }
        Err(CoreError::Message(format!(
            "최대 턴 {MAX_TURNS} 초과 — 중단"
        )))
    }
    ...
    async fn run_tool_call(
        ...
        let Some(tool) = self.registry.get(&call.name) else {
            ...
            history.push(Message {
                role: "tool".into(),
                content: format!("[{}] 오류: {msg}", call.name),
            });
            return Ok(());
        };
        ...
        let action = Action {
            tool: call.name.clone(),
            category: tool.category(&call.args),
            app,
            target: target.clone(),
        };
        ...
        let info = ActionInfo {
            ...
            risk: summary_of(&action),
        };
        match verdict {
            Verdict::Allow => {
                self.execute_and_record(session, tool, call, history, emit);
            }
            Verdict::Ask { reason } => {
                ...
                match outcome {
                    ...
                    ApprovalOutcome::Deny => {
                        ...
                        history.push(Message {
                            role: "tool".into(),
                            content: format!("[{}] 거절됨: {msg}", call.name),
                        });
                    }
                }
            }
            Verdict::Deny { reason } => {
                ...
                history.push(Message {
                    role: "tool".into(),
                    content: format!("[{}] 거부됨: {msg}", call.name),
                });
            }
        }
        Ok(())
...
fn summary_of(a: &Action) -> String {
    format!("{} {}", a.tool, a.target.clone().unwrap_or_default())
        .trim()
        .into()
}
