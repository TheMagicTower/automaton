...
struct ChatResp { choices: Vec<Choice>, usage: Option<TokenUsage> }
#[derive(serde::Deserialize)]
...
#[derive(serde::Deserialize)]
struct StreamChunk {
    #[serde(default)] choices: Vec<StreamChoice>, // usage-only 마지막 청크는 빈 choices
    usage: Option<TokenUsage>,
}
#[derive(serde::Deserialize)]
struct StreamChoice { delta: Option<StreamDelta> }
#[derive(serde::Deserialize)]
struct StreamDelta { content: Option<String>, tool_calls: Option<Vec<StreamToolCall>> }
#[derive(serde::Deserialize)]
struct StreamToolCall { index: usize, function: Option<StreamFunction> }
#[derive(serde::Deserialize)]
struct StreamFunction { name: Option<String>, arguments: Option<String> }

#[derive(Default, Clone)]
struct ToolAccum { name: String, args: String }
...
impl<S> SseState<S> {
    ...
    fn feed(&mut self, line: &str) {
        ...
        let Ok(chunk) = serde_json::from_str::<StreamChunk>(payload) else { return }; // 알 수 없는 청크 형식 무시
        if let Some(u) = chunk.usage { self.usage = Some(u); }
        for choice in chunk.choices {
            let Some(delta) = choice.delta else { continue };
            if let Some(c) = delta.content {
                if !c.is_empty() { self.pending.push_back(StreamItem::Delta(c)); }
            }
            for tc in delta.tool_calls.unwrap_or_default() {
                if self.tools.len() <= tc.index { self.tools.resize(tc.index + 1, ToolAccum::default()); }
                if let Some(f) = tc.function {
                    if let Some(n) = f.name { if !n.is_empty() { self.tools[tc.index].name.push_str(&n); } }
                    if let Some(a) = f.arguments { self.tools[tc.index].args.push_str(&a); }
                }
            }
        }
    }
    ...
    fn finish(&mut self) {
        for t in std::mem::take(&mut self.tools) {
            ...
            self.pending.push_back(StreamItem::ToolCall(ToolCall { name: t.name, args }));
        }
        if let Some(u) = self.usage.take() { self.pending.push_back(StreamItem::Usage(u)); }
    }
}
...
where
    ...
    E: fmt::Display,
{
    let state = SseState { bytes, buf: Vec::new(), tools: Vec::new(), pending: VecDeque::new(), usage: None, finished: false };
    futures_util::stream::unfold(state, |mut st| async move {
        loop {
            if let Some(item) = st.pending.pop_front() { return Some((Ok(item), st)); }
            if st.finished { return None; }
            match st.buf.iter().position(|&b| b == b'\n') {
                ...
                None => match st.bytes.next().await {
                    ...
                    Some(Err(e)) => {
                        st.finished = true;
                        return Some((Err(CoreError::Provider(format!("스트림 읽기 실패: {e}"))), st));
                    }
                    None => { st.finished = true; st.finish(); } // [DONE] 없이 종결된 스트림 — 누적분 확정
                },
            }
        }
    })
...
impl Provider for OpenAiCompat {
    async fn complete(&self, req: CompletionRequest) -> Result<Vec<StreamItem>, CoreError> {
        ...
        if !resp.status().is_success() { return Err(Self::error_for_status(resp).await); }
        let raw = resp.text().await.map_err(|e| CoreError::Provider(format!("응답 읽기 실패: {e}")))?;
        let chat: ChatResp = serde_json::from_str(&raw).map_err(|e| CoreError::Provider(format!("응답 파싱 실패: {e} — 본문: {}", &raw.chars().take(200).collect::<String>())))?;
        ...
        if req.track_usage {
            if let Some(u) = chat.usage { items.push(StreamItem::Usage(u)); }
        }
        Ok(items)
    }
...
mod tests {
    ...
    async fn sse_content_deltas_stream_in_order() {
        ...
        assert_eq!(items, vec![StreamItem::Delta("안녕".into()), StreamItem::Delta("하세요".into())]);
    }
    ...
    async fn sse_tool_call_fragments_accumulate_then_flush() {
        ...
        let raw = format!(
            "{}{}data: [DONE]\n\n",
            chunk(serde_json::json!({"choices":[{"delta":{"tool_calls":[{"index":0,"function":{"name":"fs.write","arguments":"{\"pa"}}]}}]})),
            chunk(serde_json::json!({"choices":[{"delta":{"tool_calls":[{"index":0,"function":{"arguments":"th\":\"a.txt\"}"}}]}}]})),
        );
        let items = collect(sse_items(chunks(vec![raw.as_bytes().to_vec()]))).await;
    ...
    async fn sse_usage_only_final_chunk_emits_usage() {
        ...
        assert_eq!(items, vec![
            ...
            StreamItem::Usage(TokenUsage { prompt_tokens: 10, completion_tokens: 5, total_tokens: 15 }),
        ]);
    }
