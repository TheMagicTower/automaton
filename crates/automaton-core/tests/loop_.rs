...
impl ApprovalGate for AutoGate {
    async fn decide(&self, _session: &str, _a: ActionInfo) -> ApprovalOutcome {
        self.0.clone()
    }
}
...
fn tmp(name: &str) -> std::path::PathBuf {
    let p = std::env::temp_dir()
        .join(format!("automaton-core-{}/", std::process::id()))
        .join(name);
...
fn builtin_profiles_resolve_every_tool_in_matching_registry() {
    ...
    for mode in [Mode::Code, Mode::Mac, Mode::Chat] {
        ...
        for n in &profile.tools {
            assert!(
                reg.get(n).is_some(),
                "{mode:?} 프로파일의 {n}이(가) unified_set에 없음"
            );
        }
    }
}
...
async fn run(provider: Box<dyn Provider>, gate: Box<dyn ApprovalGate>, user: &str) -> Vec<Event> {
    ...
    lp.run_turn("s1", &mut history, user.to_string(), &mut emit)
        .await
        .unwrap();
    events
}
...
async fn text_only_turn_streams_deltas() {
    ...
    let ev = run(
        Box::new(p),
        Box::new(AutoGate(ApprovalOutcome::Approve)),
        "인사해줘",
    )
    .await;
    assert!(
...
async fn usage_item_emits_usage_event_and_not_history() {
    ...
    assert!(
        evs.iter()
            .any(|e| matches!(e, Event::Usage { usage, .. } if usage.total_tokens == 10))
    );
}
...
async fn provider_stream_override_survives_dyn_forwarding() {
    let p = Streaming {
        items: vec![StreamItem::Delta("청크".into())],
    };
    ...
    assert!(
        evs.iter()
            .any(|e| matches!(e, Event::StreamDelta { delta, .. } if delta == "청크"))
    );
}
    ...
    let p = Scripted {
        turns: Mutex::new(vec![
            vec![StreamItem::ToolCall(ToolCall {
                name: "fs.write".into(),
                args: json!({"path": tmp("a.txt"), "content": "brass"}),
            })],
...
async fn denied_approval_leaves_file_and_reports_failure() {
    ...
    let ev = run(
        Box::new(p),
        Box::new(AutoGate(ApprovalOutcome::Deny)),
        "삭제해",
    )
    .await;
    assert!(
...
async fn policy_deny_skips_gate_entirely() {
    ...
    let p = Scripted {
        turns: Mutex::new(vec![
            vec![StreamItem::ToolCall(ToolCall {
                name: "fs.read".into(),
                args: json!({"path": tmp("Password.kdbx")}),
            })],
    ...
    assert!(ev.iter().any(
        |e| matches!(e, Event::ToolResult { summary, .. } if summary.contains("민감 입력 필드"))
    ));
}
...
async fn unknown_tool_yields_error_result_and_loop_continues() {
    ...
    assert!(
        ev.iter()
            .any(|e| matches!(e, Event::StreamDelta { delta, .. } if delta == "계속"))
    );
}
...
async fn runaway_provider_stops_at_max_turns() {
    ...
    let turns: Vec<Vec<StreamItem>> = (0..40)
        .map(|_| {
            vec![StreamItem::ToolCall(ToolCall {
                name: "fs.read".into(),
    ...
    let mut history = vec![];
    let r = lp
        .run_turn("s1", &mut history, "계속해".into(), &mut |_| {})
        .await;
    assert!(r.is_err());
...
async fn triple_consecutive_failures_abort_turn() {
    ...
    let p = Scripted {
        turns: Mutex::new(vec![t(), t(), t(), vec![StreamItem::Delta("x".into())]]),
        call: Mutex::new(0),
    };
    let lp = AgentLoop::new(
        Box::new(p),
    ...
    let mut history = vec![];
    let r = lp
        .run_turn("s1", &mut history, "고장".into(), &mut |_| {})
        .await;
    assert!(r.is_err());
...
async fn decoy_target_does_not_bypass_sensitive_path_deny() {
    ...
    let p = Scripted {
        turns: Mutex::new(vec![
            vec![StreamItem::ToolCall(ToolCall {
                name: "fs.read".into(),
                args: json!({"path": tmp("Password.kdbx"), "target": "benign.txt"}),
            })],
    ...
    let p = Scripted {
        turns: Mutex::new(vec![
            vec![StreamItem::ToolCall(ToolCall {
                ...
                args: json!({"command": "cat ~/Passwords.kdbx", "path": "notes.txt"}),
            })],
            vec![StreamItem::Delta("거부됨 확인".into())],
        ]),
        call: Mutex::new(0),
