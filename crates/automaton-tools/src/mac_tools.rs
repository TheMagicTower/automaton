...
fn run_osascript(script: &str, err_context: &str) -> Result<String, ToolError> {
    ...
    if !out.status.success() {
        return Err(ToolError::Message(format!("{err_context}: 접근성 권한 확인 필요 ({})", String::from_utf8_lossy(&out.stderr))));
    }
...
pub struct AxElement {
    ...
    #[serde(default)] pub w: Option<f64>,
    #[serde(default)] pub h: Option<f64>,
    #[serde(default)] pub truncated: Option<bool>,
    #[serde(default)] pub children: Option<Vec<AxElement>>,
}
...
impl Tool for AxListElements {
    fn name(&self) -> &'static str { "ax.list_elements" }
    fn description(&self) -> &'static str { "최전면 앱의 UI 요소 트리를 JSON으로 반환 (role·title·좌표·크기 포함, 깊이 3 제한)" }
    fn parameters_schema(&self) -> Value {
    ...
    fn execute(&self, _args: &Value) -> Result<String, ToolError> {
        ...
        if out == "null" {
            return Err(ToolError::Message("최전면 앱에 접근 가능한 창이 없음".into()));
        }
        Ok(out)
    }
}
...
impl AxHit {
    ...
    pub fn center(&self) -> Option<(f64, f64)> {
        ...
        let (dx, dy) = match (self.w, self.h) { (Some(w), Some(h)) => (w / 2.0, h / 2.0), _ => (0.0, 0.0) };
        Some((x + dx, y + dy))
    }
}
...
impl InputClickElement {
    pub fn parse_hit(&self, raw: &str) -> Result<AxHit, ToolError> {
        serde_json::from_str(raw).map_err(|e| ToolError::Message(format!("AX 검색 결과 파싱 실패: {e}")))
    }
}
impl Tool for InputClickElement {
    fn name(&self) -> &'static str { "input.click_element" }
    fn description(&self) -> &'static str { "최전면 앱에서 name(제목/설명 부분일치) 또는 role(정확일치)로 UI 요소를 찾아 중심 좌표를 클릭 — 대상 요소의 AX 역할·이름을 target 인자로 전달해야 정책 엔진이 민감 필드(비밀번호 등)를 검사할 수 있다" }
    fn parameters_schema(&self) -> Value {
    ...
    fn execute(&self, args: &Value) -> Result<String, ToolError> {
        ...
        let encode = |s: &str| serde_json::to_string(s).map_err(|e| ToolError::Message(format!("인자 인코딩 실패: {e}")));
        let script = r#"
            ObjC.import('Foundation');
            const se = Application('System Events');
            const p = se.processes.whose({frontmost: true})[0];
        ...
        if raw == "null" {
            return Err(ToolError::Message(format!("요소를 찾지 못함 (name='{name}', role='{role}')")));
        }
        let hit = self.parse_hit(&raw)?;
        let (x, y) = hit.center().ok_or_else(|| ToolError::Message(format!("요소에 좌표가 없음: '{}'", hit.title)))?;
        cg_click(x, y)?;
        Ok(format!("({x:.0},{y:.0}) '{}'({}) 클릭 완료", hit.title, hit.role))
    }
}
