...
fn arg_str(args: &Value, key: &str) -> Result<String, ToolError> {
    args.get(key)
        .and_then(|v| v.as_str())
        .map(String::from)
        .ok_or_else(|| ToolError::Message(format!("{key} 인자 누락")))
}
...
impl Tool for FsRead {
    ...
    fn execute(&self, args: &Value) -> Result<String, ToolError> {
        ...
        let s = std::fs::read_to_string(&path)
            .map_err(|e| ToolError::Message(format!("읽기 실패 {path}: {e}")))?;
...
impl Tool for FsReadLines {
    ...
    fn description(&self) -> &'static str {
        "파일을 '줄번호:내용' 형식으로 반환 (edit.replace_lines로 줄 지정 편집 시 근거로 사용)"
    }
    fn parameters_schema(&self) -> Value {
        serde_json::json!({"type":"object","properties":{"path":{"type":"string"}},"required":["path"]})
    }
...
impl Tool for FsWrite {
    ...
    fn execute(&self, args: &Value) -> Result<String, ToolError> {
        ...
        std::fs::write(&path, &content)
            .map_err(|e| ToolError::Message(format!("쓰기 실패 {path}: {e}")))?;
...
impl Tool for FsDelete {
    ...
    fn execute(&self, args: &Value) -> Result<String, ToolError> {
        ...
        std::fs::remove_file(&path)
            .map_err(|e| ToolError::Message(format!("삭제 실패 {path}: {e}")))?;
...
impl Tool for FsMkdir {
    ...
    fn execute(&self, args: &Value) -> Result<String, ToolError> {
        ...
        std::fs::create_dir_all(&path)
            .map_err(|e| ToolError::Message(format!("디렉터리 생성 실패 {path}: {e}")))?;
...
impl Tool for FsMove {
    ...
    fn execute(&self, args: &Value) -> Result<String, ToolError> {
        ...
        std::fs::rename(&path, &dest)
            .map_err(|e| ToolError::Message(format!("이동 실패 {path} → {dest}: {e}")))?;
...
impl Tool for EditApply {
    ...
    fn execute(&self, args: &Value) -> Result<String, ToolError> {
        ...
        let s = std::fs::read_to_string(&path)
            .map_err(|e| ToolError::Message(format!("읽기 실패 {path}: {e}")))?;
        if !s.contains(&find) {
            return Err(ToolError::Message(format!("찾을 문자열 없음: {find}")));
        }
...
impl Tool for EditReplaceLines {
    ...
    fn description(&self) -> &'static str {
        "start~end 줄(1-based, end 포함)을 content 줄들로 교체한다 — 빈 content면 해당 줄 삭제, fs.read_lines 출력의 줄번호를 그대로 쓴다"
    }
    fn parameters_schema(&self) -> Value {
    ...
    fn execute(&self, args: &Value) -> Result<String, ToolError> {
        ...
        if start == 0 || start > end || end > total {
            return Err(ToolError::Message(format!(
                "잘못된 줄 범위 {start}~{end} (전체 {total}줄, 1-based)"
            )));
        }
