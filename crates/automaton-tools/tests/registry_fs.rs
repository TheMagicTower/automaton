...
fn tmp(name: &str) -> std::path::PathBuf {
    let p = std::env::temp_dir()
        .join(format!("automaton-tools-{}/", std::process::id()))
        .join(name);
...
fn fs_write_then_read_roundtrip() {
    let p = tmp("round.txt");
    FsWrite
        .execute(&json!({"path": p, "content": "홍브라스 시대"}))
        .unwrap();
...
fn fs_grep_reports_line_numbers() {
    ...
    let out = FsGrep
        .execute(&json!({"path": p, "pattern": "brass"}))
        .unwrap();
...
fn fs_delete_removes_file_and_declares_destructive() {
    ...
    assert_eq!(
        FsDelete.category(&json!({"path": p})),
        automaton_policy::Category::Destructive
    );
...
fn fs_read_declares_read_category_and_missing_file_is_error() {
    assert_eq!(
        FsRead.category(&json!({})),
        automaton_policy::Category::Read
    );
...
fn action_context_extracts_command_as_target() {
    let (app, target) =
        automaton_tools::action_context("shell.exec", &json!({"command": "echo test"}));
    assert_eq!(app, None);
...
fn action_context_prioritizes_operational_args_over_decoy_target() {
    ...
    let (_, target_cmd) = automaton_tools::action_context(
        "shell.exec",
        &json!({"command": "rm -rf ~", "target": "cargo test", "path": "notes.txt"}),
    );
    assert_eq!(target_cmd, Some("rm -rf ~".to_string()));

    let (_, target_path) = automaton_tools::action_context(
        "fs.read",
        &json!({"path": "Passwords.kdbx", "target": "notes.txt", "command": "echo fake"}),
    );
    assert_eq!(target_path, Some("Passwords.kdbx".to_string()));

    let (_, target_input) = automaton_tools::action_context(
        "input.type",
        &json!({"target": "SecureTextField", "path": "notes.txt"}),
    );
    assert_eq!(target_input, Some("SecureTextField".to_string()));

    // 타입 혼동 방지: 비문자열 path가 있어도 정상 추출
    let (_, target_type) = automaton_tools::action_context(
        "shell.exec",
        &json!({"command": "cat secret", "path": 123}),
    );
...
fn fs_read_lines_declares_read_and_missing_file_is_error() {
    assert_eq!(
        FsReadLines.category(&json!({})),
        automaton_policy::Category::Read
    );
    assert!(
        FsReadLines
            .execute(&json!({"path": tmp("없는파일-줄읽기")}))
            .is_err()
    );
}
...
fn edit_replace_lines_replaces_inclusive_range() {
    ...
    std::fs::write(&p, "one\ntwo\nthree\nfour\n").unwrap();
    EditReplaceLines
        .execute(&json!({"path": p, "start": 2, "end": 3, "content": "B\nc"}))
        .unwrap();
...
fn edit_replace_lines_empty_content_deletes_range() {
    ...
    std::fs::write(&p, "a\nb\nc\n").unwrap();
    EditReplaceLines
        .execute(&json!({"path": p, "start": 2, "end": 2, "content": ""}))
        .unwrap();
...
fn edit_replace_lines_rejects_invalid_range_and_declares_write() {
    ...
    std::fs::write(&p, "a\nb\n").unwrap();
    assert_eq!(
        EditReplaceLines.category(&json!({"path": p})),
        automaton_policy::Category::Write
    );
    assert!(
        EditReplaceLines
            .execute(&json!({"path": p, "start": 0, "end": 1, "content": "x"}))
            .is_err()
    ); // 1-based 위반
    assert!(
        EditReplaceLines
            .execute(&json!({"path": p, "start": 2, "end": 1, "content": "x"}))
            .is_err()
    ); // start>end
    assert!(
        EditReplaceLines
            .execute(&json!({"path": p, "start": 1, "end": 99, "content": "x"}))
            .is_err()
    ); // 범위 초과
}
...
fn fs_move_relocates_file_with_content() {
    ...
    assert_eq!(
        FsMove.category(&json!({"path": src, "dest": dst})),
        automaton_policy::Category::Write
    );
    ...
    assert!(
        FsMove
            .execute(&json!({"path": tmp("없는원본"), "dest": dst}))
            .is_err()
    );
}
