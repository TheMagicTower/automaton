...
use automaton_tools::{AxListElements, AxRead, CaptureScreen, InputClick, InputClickElement, InputType, Tool};
use serde_json::json;
...
fn ax_tree_parses_nested_element_json() {
    ...
    assert_eq!((btn.role.as_str(), btn.title.as_str()), ("AXButton", "확인"));
...
fn input_click_element_classifies_by_target_f04() {
    assert_eq!(InputClickElement.category(&json!({"name":"확인","target":"button '확인'"})), Category::Input);
    assert_eq!(InputClickElement.category(&json!({"name":"확인"})), Category::External); // target 누락
    assert_eq!(InputClickElement.category(&json!({"role":"AXButton","target":"   "})), Category::External); // 빈 target
    assert_eq!(InputClickElement.category(&json!({"name":"x","target":7})), Category::External); // 비문자열 target
}

#[test]
fn input_click_element_requires_search_operand() {
    // name/role 둘 다 없으면 정책 target이 있어도 실행 불가 — osascript 실행 전에 검증
    assert!(InputClickElement.execute(&json!({"target":"button"})).is_err());
}

#[test]
fn ax_hit_center_is_bbox_middle() {
    let hit = InputClickElement.parse_hit(r#"{"role":"AXButton","title":"OK","x":10,"y":20,"w":100,"h":50}"#).unwrap();
    assert_eq!(hit.center(), Some((60.0, 45.0)));
    let bare = InputClickElement.parse_hit(r#"{"role":"AXCheckBox","x":5,"y":7}"#).unwrap();
    assert_eq!(bare.center(), Some((5.0, 7.0))); // 크기 없으면 좌상단 그대로
    let no_pos = InputClickElement.parse_hit(r#"{"role":"AXButton"}"#).unwrap();
...
fn clicks_element_found_by_role() {
    let out = InputClickElement.execute(&json!({"role":"AXButton","target":"button"})).unwrap();
