...
fn approval(tool: &str) -> Decision {
    Decision { session: "s1".into(), tool: tool.into(), target: "~/Downloads/old.zip".into(), verdict: "ask".into(), decision: "approve".into() }
}
...
fn three_approvals_yield_go_ahead_draft() {
    ...
    let drafts = a.drafts_for(&info("fs.delete", "~/Downloads/new.zip")).unwrap();
    assert!(drafts.iter().any(|d| d == "진행해"), "초안에 '진행해' 필요: {drafts:?}");
}

#[test]
fn no_history_yields_no_drafts() {
    let a = Apprentice::open(&tmp("empty")).unwrap();
    assert!(a.drafts_for(&info("fs.delete", "~/Downloads/a.zip")).unwrap().is_empty());
    assert!(DraftComposer.generate_drafts("fs.delete", "~/Downloads/a.zip", &[]).is_empty());
}

#[test]
fn other_tool_history_is_not_used() {
    let past = vec![approval("fs.write"), approval("fs.write"), approval("fs.write")];
    assert!(DraftComposer.generate_drafts("fs.delete", "~/Downloads/new.zip", &past).is_empty());
    assert!(!DraftComposer.generate_drafts("fs.write", "~/notes.md", &past).is_empty());
}
